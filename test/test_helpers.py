...
class TestDataSplitFunction(unittest.TestCase):
    def test_split_cols(self):
        df = pd.read_csv('./resources/inference_data.csv')
        X_train, X_test, y_train, y_test = data_split(df, ['Income', 'Age'], 'History of Mental Illness', 0.5)
    ...
    def test_split_target(self):
        df = pd.read_csv('./resources/inference_data.csv')
        _, _, y_train, y_test = data_split(df, ['Income', 'Age'], 'History of Mental Illness', 0.5)
